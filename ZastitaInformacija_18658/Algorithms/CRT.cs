...
namespace ZastitaInformacija_18658.Algorithms
{
    public static class CRT
    {
        public static byte[] Encrypt(byte[] data, string key)
        {
            ...
            // Store length + encrypted data (use 4 bytes per encrypted byte)
            byte[] result = new byte[data.Length * 4 + 4];
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                // Store full uint value (4 bytes) instead of reducing to a single byte
                BitConverter.GetBytes(encrypted).CopyTo(result, 4 + i * 4);
            }
        ...
        public static byte[] Decrypt(byte[] data, string key)
        {
            ...
            {
                // Read the full uint value (4 bytes)
                uint encrypted = BitConverter.ToUInt32(data, 4 + i * 4);
                uint decrypted = 0;
        ...
        private static uint GetInverseProduct(uint[] moduli, int index)
        {
            ...
            uint productMod = product % moduli[index];
            uint inverse = ModInverse(productMod, moduli[index]);
            return (product * inverse) % GetProduct(moduli);
        }
